...
def analyze_bridge_dynamics(lyrics):
    if not lyrics:
        return None
    ...
    for part in parts:
        ...
        if part.startswith('[') and part.endswith(']'):
            label = part.lower()
            if 'verse' in label: current_label = 'verse'
            elif 'bridge' in label: current_label = 'bridge'
            elif 'chorus' in label: current_label = 'chorus'
            else: current_label = 'other'
        elif current_label in sections:
            sections[current_label] += " " + part

    # 2. Sentiment Analysis per section
    # We use .get() to handle cases where a section might be missing
    v_sent = analyzer.polarity_scores(sections['verse'])['compound'] if sections['verse'] else 0
    b_sent = analyzer.polarity_scores(sections['bridge'])['compound'] if sections['bridge'] else 0
    c_sent = analyzer.polarity_scores(sections['chorus'])['compound'] if sections['chorus'] else 0
...
def main():
    ...
    conn.execute("CREATE OR REPLACE TABLE dim_bridge_metrics AS SELECT * FROM res_df")
    
    # We also update the master_training_data for the ML model
    # (Assuming you re-run your Master Table creation after rescraping)
    ...
    print(f"Most Dramatic Bridge Shift: {top_shift[0]} ({top_shift[1]}) with {round(top_shift[2], 3)} shift")
